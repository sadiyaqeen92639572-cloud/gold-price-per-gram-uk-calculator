...
// baked-in SVG chart on /gold-price-history-uk/ never drifts from history.json.
...
function buildSVG(history, field) {
  ...
  const firstDate = new Date(pts[0].t).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' });
  const lastDate = new Date(pts[pts.length - 1].t).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' });
...
function buildStats(history, field) {
  ...
  return {
    ...
    latest: latest[field],
    changeAbs, changePct,
    fromDate: new Date(first.t).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
    toDate: new Date(latest.t).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' }),
    points: pts.length,
  };
}
